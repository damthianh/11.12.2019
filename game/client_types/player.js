...
module.exports = function(treatmentName, settings, stager, setup, gameRoom) {
    ...
    stager.setOnInit(function() {
        var header, frame;
        //Change today
       // var income; //add new income
        var COINS; //put comment
//end of change
        console.log('INIT PLAYER!');
/////////change today

        COINS = node.game.settings.INITIAL_COINS;//put comment
        ...
        this.isValidDemand = this.isValidContribution = function (n) {
            return false !== JSUS.isInt(n, 0, node.game.income); //it was COINS before
        };
            ...
            if (checkResults.success) {
                ...
                if (checkResults.errContrib) {
                    ...
                    if ('number' !== typeof node.game.oldContrib) {
                        contrib = JSUS.randomInt(-1, node.game.income); //20 before
                    } else {
                ...
                if (checkResults.errDemand) {
                    ...
                    if ('number' !== typeof node.game.oldDemand) {
                        demand = JSUS.randomInt(-1, node.game.income); //20 before
                    } else {
        ...
        this.checkInputs = function () {
            ...
            if (!node.game.isValidContribution(contrib)) {
                ...
                errorC.innerHTML = 'Invalid contribution. ' +
                    'Please enter a number between 0 and ' + node.game.income; //it was COINS before instead of node.game.income
...
//                }
            ...
            save = node.game.income - node.game.oldContrib; //it was COINS  - node.game.oldContrib before
        ...
        this.displaySummaryPrevRound = function () {
            ...
            if ('number' === typeof node.game.oldContrib) {
                ...
                save = node.game.income - node.game.oldContrib;  //it was COINS before
                groupReturn = node.game.oldPayoff -save; //before groupReturn = node.game.oldPayoff - save;
    ...
    stager.extendStep('bid', {
        ...
        cb: function() {
           ...
           //Here is something I add today
           node.on.data('income', function (msg) {
               var income;
               //income.innerHTML=alert(msg.data);
